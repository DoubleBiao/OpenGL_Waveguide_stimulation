...
public:
	...
	//cube_surface(glm::vec3 vertices[]);
	void init(glm::vec3 vex[]);
	void initwaveguide(GLfloat mur, GLfloat epsion, GLfloat m, GLuint n, GLfloat omiga)
	{
		const GLfloat PI = 3.1415926535898;
		_ele_color.init_waveguide(mur, epsion,m,n,omiga * 2 * PI);
	}
	...
	{
		_ele_color.changewavemode(m,n);
	}
	void loadtime(GLfloat t)
	{
		_ele_color.loadtime(t);
	}

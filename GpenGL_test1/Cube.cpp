...
cube::cube()
{
	globject_part *part = new globject_part[6];
	for(int i = 0; i < 6 ;i++)
	{
		part[i]._color = glm::vec3(1.0f, 0.5f, 0.2f);
		part[i].assign_index(__cube__indices[i], 6);
	}
	set_part(part, 6);
}


cube::cube(glm::vec3  ver[], int vertexnum)
{
	globject_part *part = new globject_part[6];
	for(int i = 0; i < 6 ;i++)
	{
...
cube::cube(glm::vec3  ver[], int vertexnum, glm::vec3 color)
{
	globject_part *part = new globject_part[6];
	for(int i = 0; i < 6 ;i++)
	{

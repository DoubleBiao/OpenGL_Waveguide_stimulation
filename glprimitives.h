...
public:
	...
	void calparameters()
	{
		...
		const GLfloat C = 299792.458e3;
		_api = PI/(glm::length(_rowdir)*1e-2); _bpi = PI/(glm::length(_layerdir)*1e-2);

		GLfloat hsquare = (_m * _api) * (_m * _api) + (_n * _bpi) * (_n * _bpi);
		GLfloat k = _omiga / C * sqrt(_mur * _epsionr);
		_beta = k * sqrt(1 - hsquare/k/k);
		_betaoverh2 = _beta / hsquare;
	}
	void init_waveguide(GLfloat mur, GLfloat epsion, GLuint m, GLuint n, GLfloat omiga)
	{
		_omiga = omiga; _mur = mur; _epsionr = epsion; _m = m; _n = n;
		calparameters();
	}

	void loadtime(GLfloat t)
	{
		_phase = t * _omiga * Time_AMP; 
	}
	void changewavemode(GLuint m, GLuint n)
	{
		_m = m; _n = n;
		calparameters();
	}
	...
	void writeobjectanduniform()
	{
		...
		GLint betaoverh2Loc = glGetUniformLocation(_shader.Program, "betaoverh2");
		GLint phaseLoc = glGetUniformLocation(_shader.Program, "phase");
		GLint apiLoc = glGetUniformLocation(_shader.Program, "api");
		GLint bpiLoc = glGetUniformLocation(_shader.Program, "bpi");
		GLint betaLoc = glGetUniformLocation(_shader.Program, "beta");

		glUniform1f(betaoverh2Loc,_betaoverh2);
		glUniform1f(phaseLoc,_phase);
		glUniform1f(apiLoc,_api);
		glUniform1f(bpiLoc,_bpi);
		glUniform1f(betaLoc,_beta);
	}
};
#endif
